"""Command-line interface for Distortropy."""
...
from distortropy.Backend.pipeline import (
    build_state_from_cif_info,
    build_state_from_parent_state,
)
from distortropy.Backend.modes.mode_detail_text import render_mode_detail_text
from distortropy.Backend.parent import (
    build_generic_parent_state,
    build_parent_state_from_cif_info,
    read_cif_summary,
)
from distortropy.Backend.reciprocal.k_points import select_k_points
from distortropy.case_input import (
    DistortropyCase,
    case_from_cli,
    case_from_json,
    load_case,
...
from distortropy.cli_output import compact_opd_result, compact_result
from distortropy.cli_parser import parser
...
def _direct_parent_state(args: argparse.Namespace) -> dict[str, Any] | None:
    ...
    site_free_strain = (
        args.command in {"irreps", "opds", "invariants"}
...
def _load_cli_case(path: Path) -> DistortropyCase:
    if str(path) != "-":
...
def _direction_mode_selection(
    ...
    if not isinstance(payload, dict) or payload.get("schema") != "distortropy.cli.directions":
...
def _cli_case(args: argparse.Namespace) -> DistortropyCase:
    if args.command in {"modes", "invariants", "run"} and args.case is not None:
        ...
        if stage == "modes" and case.space_group is not None and not case.wyckoff:
...
def _invariant_result_from_factors(
    *,
    ...
    from distortropy import _invariant_service, handle_api_post
    ...
    return {
        "schema": "distortropy.cli.invariants",
...
def _state_with_invariant_factors(state: dict[str, Any]) -> dict[str, Any]:
    ...
    from distortropy import _complete_invariant_factors
...
def main(argv: Sequence[str] | None = None) -> int:
    ...
    try:
        if args.command == "serve":
            from distortropy.server import serve

            serve(host=args.host, port=args.port, open_browser=args.open_browser)
            return 0
        if args.command == "settings":
            from distortropy.Backend.settings import international_settings
        ...
        if args.command == "directions":
            if len(args.basis) != 3:
                raise ValueError("--basis must be repeated exactly three times")
            from distortropy.Backend.directions import (
                compatible_directions,
                compatible_magnetic_directions,
            )

            basis = tuple(
                _exact_vector(value, f"--basis row {index}")
                for index, value in enumerate(args.basis, start=1)
            )
        ...
        if args.command == "combine-modes":
            from distortropy.cli_mode_combination import (
        ...
        if args.command == "invariants" and args.from_modes is not None:
            ...
            from distortropy.cli_mode_combination import load_compact_modes
