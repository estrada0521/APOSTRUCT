"""Standalone localhost server for the Distortropy frontend and API."""
...
def _asset_path(path: str) -> Path | None:
    if path in {"/", "/index.html", "/tools/distortropy", "/tools/distortropy/"}:
        relative = "index.html"
    elif path.startswith("/tools/distortropy/"):
        relative = path.removeprefix("/tools/distortropy/")
    else:
...
def make_handler():
    class Handler(BaseHTTPRequestHandler):
        ...
        def do_GET(self) -> None:
            ...
            if parsed.path.startswith("/api/distortropy/"):
                endpoint = parsed.path.removeprefix("/api/distortropy/")
                try:
        ...
        def do_POST(self) -> None:
            ...
            if parsed.path == "/shutdown":
                ...
                return
            if not parsed.path.startswith("/api/distortropy/"):
                self._send(404, "Not Found")
                return
            try:
    ...
    class Server(ThreadingHTTPServer):
        def server_close(self) -> None:
            if _saved_debug_cases_available():
                from distortropy.debug.jobs import cancel_all_debug_jobs
