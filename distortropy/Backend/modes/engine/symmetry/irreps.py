...
from distortropy.Backend.modes.engine.records import (
    LittleKVectorRecord,
)
...
class IrrepMixin:
    ...
    def project_vector_bridge_weight_view_for_record(
        ...
        *,
        ...
        return self._project_vector_bridge_weight_view(
            int(gid),
            tuple(int(value) for value in record),
            int(case.sg),
            tuple(Fraction(value) for value in case.k_params),
            int(output_length),
        )

    @lru_cache(maxsize=32768)
    def _project_vector_bridge_weight_view(
        self,
        gid: int,
        record: tuple[int, int, int, int, int],
        sg: int,
        ...
        # Bridge phases depend only on the parent SG and runtime k parameters.
        phase_case = Case(
            sg=sg,
            wyckoff="",
            k_label="",
        ...
        matrix = self._bridge_irrep_matrix_for_record(gid, record, phase_case)
