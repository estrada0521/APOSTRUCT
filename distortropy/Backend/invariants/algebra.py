...
def polynomial_terms(
    ...
    """Return exact coefficients and exponent vectors in display-term order."""

    return tuple(
        (sstr(coefficient), tuple(int(value) for value in exponent))
        for exponent in sorted(polynomial, reverse=True)
        if (coefficient := _sympy_scalar(polynomial[exponent])) != 0
    )
