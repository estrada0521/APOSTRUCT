...
from .source import (
    ...
    _rational_entry,
    _synchronized_operation_records,
...
def _project_parametric_records(
    projection: InvariantSource,
    *,
    space_group: int,
    gids: Sequence[int],
    source_parameters: Sequence[Sequence[int] | None],
    direction_blocks: Sequence[ExactMatrix],
) -> tuple[tuple[ExactMatrix, ...], tuple[int, ...]]:
    """Project synchronized operation records without retaining full matrices."""

    block_dimensions = tuple(len(block[0]) for block in direction_blocks)
    norms: list[tuple[object, ...]] = []
    supports: list[tuple[tuple[int, ...], ...]] = []
    for block in direction_blocks:
        width = len(block[0])
        if any(len(row) != width for row in block):
            raise ValueError("OPD direction matrix is ragged")
        block_supports = tuple(
            tuple(row for row in range(len(block)) if block[row][column])
            for column in range(width)
        )
        block_norms = tuple(
            _coefficient(sum(
                (
                    block[row][column] * block[row][column]  # type: ignore[operator]
                    for row in block_supports[column]
                ),
                Fraction(0),
            ))
            for column in range(width)
        ...
        for left in range(width):
            for right in range(left):
                overlap = sum(
                    (
                        block[row][left] * block[row][right]  # type: ignore[operator]
                        for row in set(block_supports[left]) & set(block_supports[right])
                    ),
                    Fraction(0),
                )
                if overlap:
                    raise ValueError("combined OPD direction columns are not orthogonal")
        if any(norm == 0 for norm in block_norms):
            raise ValueError("combined OPD direction contains a zero column")
        norms.append(block_norms)
        supports.append(block_supports)

    records = _synchronized_operation_records(
        projection.source_data,
        int(space_group),
        gids,
        tuple(() if values is None else values for values in source_parameters),
    )
    variable_count = sum(block_dimensions)
    projected = []
    for record in records:
        rows = [
            [Fraction(0) for _column in range(variable_count)]
            for _row in range(variable_count)
        ]
        offset = 0
        for gid, parameters, block, block_norms, block_supports in zip(
            gids,
            source_parameters,
            direction_blocks,
            norms,
            ...
            raw = projection.get_irreps_matrix_for_record(
                int(gid),
                record,
                kparam=parameters,
            )
            exact_entry = _rational_entry if parameters is None else _quadratic3_entry
            width = len(block[0])
            for left in range(width):
                for right in range(width):
                    coefficient = sum(
                        (
                            block[row][left]
                            * exact_entry(raw[row, column])
                            * block[column][right]  # type: ignore[operator]
                            for row in block_supports[left]
                            for column in block_supports[right]
                        ),
                        Fraction(0),
                    )
                    rows[offset + left][offset + right] = _coefficient(
                        coefficient / block_norms[left]  # type: ignore[operator]
                    )
            offset += width
        projected.append(tuple(tuple(row) for row in rows))
    return tuple(projected), block_dimensions
...
def restricted_parametric_irrep_action(
    ...
    *,
    projection_source: InvariantSource | None = None,
    selected_direction_matrices: Sequence[ExactMatrix | None] | None = None,
) -> tuple[tuple[ExactMatrix, ...], tuple[int, ...]]:
    ...
    if any(type(value) is not int or value < 1 for value in domain_values):
        raise ValueError("domain values must be positive integers")
    selected_blocks = tuple(selected_direction_matrices or (None,) * len(labels))
    if len(selected_blocks) != len(labels):
        raise ValueError("selected direction matrices must match the selected factors")
    ...
    direction_blocks = []
    for gid, kparam, direction, domain, selected_block in zip(
        gids,
        source_parameters,
        opd_labels,
        domain_values,
        ...
        if selected_block is not None:
            direction_blocks.append(selected_block)
            continue
        little = projection.little_record_by_gid(gid)
        if int(little.old_id) > 0:
            row_id, block = _exact_direction_matrix(projection, gid, direction)
    ...
    return _project_parametric_records(
        projection,
