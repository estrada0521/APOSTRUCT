"""Invariant-facing operations backed by the shared domain kernel."""
...
def direction_domain_from_subgroup(
    source,
    *,
    sg: int,
    gid: int,
    ...
    """Match a carrier subspace to the Source domain order of one OPD row."""

    target_matrix = np.asarray(target, dtype=complex)
    candidate_matrix = np.asarray(candidate, dtype=complex)
    target_projector = target_matrix @ np.linalg.pinv(target_matrix)
    count = domain_count_from_subgroup(
        source.source_data,
        sg=int(sg),
        child_sg=int(child_sg),
        basis=basis,
    ...
    for domain in range(1, count + 1):
        ...
        if domain > 1:
            ...
            transformed = (
                np.asarray(
                    source.get_irreps_matrix_for_record(
                        int(gid),
                        record,
                        kparam=tuple(kparam),
                    ),
        ...
        projector = transformed @ np.linalg.pinv(transformed)
        if np.max(np.abs(target_projector - projector)) <= float(tolerance):
            return domain
    return None
