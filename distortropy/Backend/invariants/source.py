...
def _quadratic3_record_matrix(
    ...
    for row in range(rows):
        ...
        for column in range(columns):
            values.append(_quadratic3_entry(raw[row, column]))
        out.append(tuple(values))
    return tuple(out)


def _quadratic3_entry(value: object) -> Fraction | ExactAlgebraicNumber:
    scalar = complex(value)
    if abs(scalar.imag) > 1e-12:
        raise ValueError("real DISPLAY INVARIANT matrix has a complex entry")
    exact = ExactAlgebraicNumber.from_float(scalar.real)
    return exact.as_fraction() if exact.is_rational else exact


def _translation_kernel_basis(
    data: object,
    gids: Sequence[int],
    source_parameters: Sequence[Sequence[int]],
) -> tuple[int, ...]:
    """Return the exact common translation kernel of synchronized irreps."""

    vectors = tuple(
        vector
        for gid, parameters in zip(gids, source_parameters, strict=True)
        for vector in data._little_k_star_vectors(int(gid), tuple(parameters))  # noqa: SLF001
    )
    denominator = lcm(*(
        value.denominator
    ...
    def allowed(vector: tuple[int, int, int]) -> bool:
        return all(
            sum(kvector[axis] * vector[axis] for axis in range(3)).denominator == 1
            for kvector in vectors
        )

    periods = []
    for axis in range(3):
        for value in range(1, denominator + 1):
            vector = [0, 0, 0]
            vector[axis] = value
            if allowed(tuple(vector)):  # type: ignore[arg-type]
                periods.append(value)
                break
        else:
            raise ValueError(f"translation kernel has no period along axis {axis}")

    rows: list[list[int]] = []
    for axis in range(3):
        if axis == 0:
            candidates = ((x, 0, 0) for x in range(1, periods[0] + 1))
        elif axis == 1:
            candidates = (
                (x, y, 0)
                for y in range(1, periods[1] + 1)
                for x in range(periods[0])
            )
        else:
            candidates = (
                (x, y, z)
                for z in range(1, periods[2] + 1)
                for y in range(periods[1])
                for x in range(periods[0])
            )
        row = next((list(vector) for vector in candidates if allowed(vector)), None)
        if row is None:
            raise ValueError(f"translation kernel has no basis row {axis}")
        rows.append(row)
    return tuple(data._reduce_basis_rows(rows))  # noqa: SLF001


def _synchronized_operation_records(
    data: object,
    space_group: int,
    gids: Sequence[int],
    source_parameters: Sequence[Sequence[int]],
) -> tuple[tuple[int, int, int, int, int], ...]:
    """Enumerate the parent group modulo the exact common translation kernel.

    A spatial kernel of an irrep may still duplicate matrices.  Those fibers
    all have the same size, so retaining them leaves every Reynolds projection
    unchanged while avoiding an expensive exact matrix-group closure.
    """

    basis = _translation_kernel_basis(data, gids, source_parameters)
    fractions = get_new_fractionals(basis)
    generated = data.generate_space_group_records(int(space_group))
