...
class SourceTables:
    ...
    def default_setting_space_symbol(self, sg: int) -> str:
        ...
        tables. The public OPD table, however, displays subgroup
    ...
    def default_space_group_preferences(self) -> str:
        """Return the public default setting-preference sentence.

        The uploaded parent can still be in a non-default setting. The UI then
        adds a separate "Space-group preferences for parent" line; the
    ...
    def international_setting_records_for_space_group(self, sg: int) -> list[dict[str, Any]]:
        """Return the International-setting choices exposed by COPL.

        Source stores additional legacy and signed-permutation records after
        the International choices.  COPL uses the records through the last
        default International choice and omits legacy version-2 rows.
        """

        parent = int(sg)
        if not 1 <= parent <= 230:
            raise ValueError("space group must be an integer from 1 through 230")
        last_international = max(int(value) for value in self.space["ispace_inter_choice"])
        return [
            record
            for setting_id in self.inter_setting_ids_for_space_group(parent)
            if setting_id <= last_international
            and (record := self.inter_setting_record(setting_id))["version"] != 2
        ]

    def international_setting_record(self, sg: int, setting_id: int | None) -> dict[str, Any]:
        """Resolve one COPL-compatible setting, defaulting to the SG preference."""

        parent = int(sg)
        wanted = int(
            self.space["ispace_inter_choice"][parent - 1]
            if setting_id is None
            else setting_id
        )
        for record in self.international_setting_records_for_space_group(parent):
            if int(record["id"]) == wanted:
                return record
        raise ValueError(f"setting {wanted} is not an International setting for SG{parent}")
    ...
    @staticmethod
    def _normalized_affine4(
        matrix: Sequence[Sequence[int | Fraction]],
    ) -> list[list[Fraction]]:
        denominator = Fraction(matrix[3][3])
        if denominator == 0:
            raise ValueError("zero affine denominator")
        normalized = [
            [Fraction(matrix[row][col]) / denominator for col in range(4)]
            for row in range(4)
        ]
        for row in range(3):
            normalized[row][3] = Fraction(0)
        normalized[3][3] = Fraction(1)
        return normalized

    def subgroup_embedding_from_cinter(
        self,
        parent_sg: int,
        subgroup: int,
        ...
        *,
        ...
        """Convert a public International-setting embedding to Source PML."""

        if len(basis) != 3 or any(len(row) != 3 for row in basis):
            raise ValueError("subgroup basis must contain three 3-vectors")
        if len(origin) != 3:
            raise ValueError("subgroup origin must contain three values")
        public = [
            [Fraction(basis[row][col]) for col in range(3)] + [Fraction(0)]
            for row in range(3)
        ]
        public.append([Fraction(value) for value in origin] + [Fraction(1)])
        ...
        left = self._mat4_mul(
            self._normalized_affine4(child_inter),
            self._normalized_affine4(self._space_cinter_base_matrix(int(subgroup), 0)),
        )
        right = self._mat4_mul(
            self._normalized_affine4(self._space_cinter_base_matrix(int(parent_sg), 1)),
            self._normalized_affine4(parent_inter),
        )
        source = self._mat4_mul(
            self._mat4_mul(self._mat4_inverse_fraction(left), public),
            self._mat4_inverse_fraction(right),
        )
        source_basis = tuple(source[row][col] for row in range(3) for col in range(3))
        if any(value.denominator != 1 for value in source_basis):
            raise ValueError("subgroup embedding does not map to an integral Source basis")
        return tuple(int(value) for value in source_basis), self._reduce2_fraction(list(source[3]))
    ...
    def isotropy_subductions_for_row(self, row_id: int) -> tuple[IsotropySubduction, ...]:
        ...
        In complete mode details, selecting an OPD fixes an
    ...
    def site_point_group_hm_label(self, site_pg: int) -> str:
        return str(self.space["point_group_label_inter"][int(site_pg) - 1]).strip()
    ...
    def match_wyckoff_site(self, sg: int, site: dict[str, Any], tol: float = 1e-5) -> dict[str, Any] | None:
        ...
        if any(value is None for value in values):
            return None
        fract = tuple(float(value) for value in values)
    ...
    def match_inter_wyckoff_site(
        ...
        if not site.get("fract"):
            return None
        fract_values = tuple(float_cif_number(x) for x in site["fract"])
        source_values = tuple(
            float_cif_number(x)
            for x in site.get("_source_fract", site["fract"])
        )
        if any(value is None for value in (*fract_values, *source_values)):
            return None
        fract = tuple(float(value) for value in fract_values)
        source_fract = tuple(float(value) for value in source_values)
        wanted_mult: int | None = None
        if site.get("multiplicity"):
            match = re.search(r"\d+", str(site["multiplicity"]))
            if match:
    ...
    def site_pg_irrep_label(self, site_pg: int, pg_irrep: int) -> str:
        ...
        return str(self.iso.irreps["irrep_label_pg"][old_id - 1]).strip()
    ...
    def wyckoff_subduction_pairs_for_little_gid(
        ...
        This is the Distortropy backend equivalent of the upstream SMODES
