...
from distortropy.Backend.parent import generic_parent_catalog
...
def _saved_debug_cases_available() -> bool:
    return find_spec("distortropy.debug") is not None
...
def _invariant_service():
    from distortropy.Backend.invariants.service import invariant_service
...
def handle_api(endpoint: str, query: dict[str, list[str]]):
    ...
    if endpoint == "debug_random":
        ...
        from distortropy.debug import random_saved_branch
    ...
    if endpoint == "debug_magndata":
        _require_saved_debug_cases(endpoint)
        from distortropy.debug import magndata_case_catalog
...
def handle_api_post(endpoint: str, query: dict[str, list[str]], payload: dict[str, Any]):
    if endpoint == "debug_details":
        _require_saved_debug_cases(endpoint)
        from distortropy.debug.jobs import run_debug_job
    ...
    if endpoint == "debug_cancel":
        _require_saved_debug_cases(endpoint)
        from distortropy.debug.jobs import cancel_debug_job
    ...
    if endpoint == "debug_reveal_cif":
        _require_saved_debug_cases(endpoint)
        from distortropy.debug import reveal_saved_cif
