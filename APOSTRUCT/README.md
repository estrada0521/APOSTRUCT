...
This package implements APOSTRUCT's local crystal-symmetry pipeline in Python.
...
An existing JSON case can be loaded into a running frontend:
...
imports through `APOSTRUCT/Backend/`.

Runtime boundary:

- APOSTRUCT does not invoke external crystallographic executables.
...
apo irreps structure.cif --k DT b=1/3 --magnetic Fe
apo opds structure.cif --k R --irrep R4- --displacive O
apo modes structure.cif --k R --irrep R4- --displacive O --opd P1
apo modes structure.cif --k R --irrep R4- --displacive O --opd P1 --full-state
apo invariants structure.cif --k R --irrep R4- --displacive O --opd P2
apo opds --sg 221 --wyckoff 1a 1b 3c --k R --irrep R4- --displacive c
apo modes --sg 2 --wyckoff i:x=1/7,y=2/11,z=3/13 --k GM --irrep GM1+ --displacive i --opd P1
```
...
cat case.json | apo run --case - --upto modes
```

Use `run` for saved multi-stage work so one process owns table initialization.
Use the individual commands to inspect the vocabulary stored in a case.
