...
def load_compact_modes(path: Path) -> dict[str, Any]:
    ...
    if value.get("schema") != "APOSTRUCT.cli.modes":
        raise ValueError("expected a APOSTRUCT.cli.modes payload")
    return value


def parse_definition_weights(raw_weights: Sequence[str]) -> list[tuple[str, Fraction]]:
    if not raw_weights:
...
def combine_mode_payload(
    ...
    *,
    ...
    if payload.get("schema") != "APOSTRUCT.cli.modes":
        raise ValueError("combine-modes requires a APOSTRUCT.cli.modes payload")
    ...
    weights = parse_definition_weights(raw_weights)
    ...
    result: dict[str, Any] = {
        "schema": "APOSTRUCT.cli.mode_combination",
        "source_schema": "APOSTRUCT.cli.modes",
...
__all__ = ["combine_mode_payload", "load_compact_modes", "parse_definition_weights"]
