...
from APOSTRUCT.Backend.source.tables import SourceTables, source_tables
from APOSTRUCT.Backend.parent.generic import (
...
def _parent_origin_choice_from_cif_operations(
    sg: int,
    operations: list[str],
) -> str | None:
    """Return gemmi's origin-choice extension ('1'/'2') when ops identify ``sg``."""

    if not operations:
        return None
    try:
        cif_group = gemmi.find_spacegroup_by_ops(
            gemmi.GroupOps([gemmi.Op(str(text)) for text in operations])
        )
    except (RuntimeError, TypeError, ValueError):
        return None
    if (
        cif_group is None
        or int(cif_group.number) != int(sg)
        or cif_group.ext not in {"1", "2"}
    ):
        return None
    return str(cif_group.ext)


def _parent_setting_from_cif_operations(
    local_data: SourceTables,
    sg: int,
    operations: list[str],
    candidates: tuple[int, ...],
) -> int | None:
    origin_choice = _parent_origin_choice_from_cif_operations(sg, operations)
    if origin_choice is None:
        return None
    matches = [
        int(setting_id)
        for setting_id in candidates
        if str(local_data.inter_setting_record(int(setting_id)).get("origin"))
        == origin_choice
    ]
...
def _select_parent_inter_setting(
    ...
    # Gemmi may identify origin choice 1/2 while several axis variants share that
    # choice (e.g. Fddd).  Restrict the score pool to that origin before applying
    # default_bonus; otherwise the Source default origin-2 setting wins ties and
    # OPD origins diverge from Web for origin-1 CIFs.
    origin_choice = _parent_origin_choice_from_cif_operations(int(sg), operations)
    ...
    operation_setting = _parent_setting_from_cif_operations(
        local_data, sg, operations, candidates
    )
    if operation_setting is not None:
        _score, matches = _match_parent_inter_setting(
            local_data, sg, atom_sites, operations, operation_setting
        )
        if all(
            match is not None
            for site, match in zip(atom_sites, matches, strict=True)
            if site.get("fract")
        ):
            return operation_setting, matches
...
def _wyckoff_mapping_error(site: dict[str, Any], sg: int) -> str:
    coordinates = site.get("fract")
    if not isinstance(coordinates, list) or len(coordinates) != 3:
        return "fractional coordinates are missing"
    invalid = [value for value in coordinates if float_cif_number(value) is None]
    if invalid:
        return f"unsupported fractional coordinate value {invalid[0]!r}"
    return f"coordinates do not match a Wyckoff position in any setting of SG{sg}"
...
def build_parent_state_from_cif_info(cif_info: dict[str, Any]) -> dict[str, Any]:
    ...
    for site, selected_match in zip(
        ...
        site.pop("wyckoff_mapping_error", None)
        ...
        if match:
            ...
            site["wyckoff_mapping_error"] = _wyckoff_mapping_error(site, sg)
