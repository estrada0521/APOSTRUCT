...
from APOSTRUCT.Backend.modes import (  # noqa: E402
    build_mode_details,
)
from APOSTRUCT.Backend.reciprocal.k_points import normalize_k_params  # noqa: E402
from APOSTRUCT.Backend.reciprocal.irreps import select_irreps  # noqa: E402
from APOSTRUCT.Backend.reciprocal.k_points import select_k_points  # noqa: E402
from APOSTRUCT.Backend.isotropy.selection import (  # noqa: E402
    select_embedding_direction,
    select_order_parameter_direction,
)
from APOSTRUCT.Backend.parent import (  # noqa: E402
    ...
    *,
    ...
    for name, selected in (("displacive", displacive_sites), ("magnetic", magnetic_sites)):
        ...
        unresolved = [
            ...
            if not atoms[index - 1].get("wyckoff_row_id")
        ]
        if unresolved:
            index, site = unresolved[0]
            label = str(site.get("label") or site.get("type") or f"site {index}")
            coordinates = site.get("fract")
            reason = str(
                site.get("wyckoff_mapping_error")
                or "Wyckoff position was not resolved"
            )
            raise ValueError(
                f"cannot select {name} site {label!r} at {coordinates!r}: {reason}"
            )
    if (
        not allow_empty_distortion
        and not include_strain
        and not displacive_sites
        and not magnetic_sites
    ):
