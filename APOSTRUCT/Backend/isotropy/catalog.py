...
from APOSTRUCT.Backend.isotropy.lattice_conventions import (
...
from APOSTRUCT.Backend.source.iso_data import reciprocal_to_cinter_matrix_from_table
...
from APOSTRUCT.Backend.source.tables import SOURCE, SourceTables, source_tables
...
def _selected_static_magnetic_rows(
    rows: list[Any],
    selected_orderparam: int | str | None,
) -> list[Any]:
    if not isinstance(selected_orderparam, str):
        return rows
    label = selected_orderparam.strip()
    selected = [row for row in rows if row.orderparam_label == label]
...
def _dynamic_kparam(
    ...
    *,
    ...
    # Use the public display k-vector as the authoritative bridge.
    display_vector = str(_k_slot_info(dec, sg, kslot).get("display_kvector") or "")
    ...
    for param_index in range(min(dim, 3)):
        key = display_keys[param_index] if param_index < len(display_keys) else DISPLAY_K_PARAMS[param_index]
        raw_value = k_params.get(key)
        if raw_value is None:
            raw_value = k_params.get(DISPLAY_K_PARAMS[param_index])
        raw = raw_value
...
def _source_kparam_from_visible(
    ...
    *,
    ...
    """Convert public K parameters to Source/iso internal kparam.

    The public surface displays parametric k coordinates in its conventional
    ...
    sg = int(dec.little["little_irr_space_group"][int(gid) - 1])
    kslot = int(dec.little["little_irr_k"][int(gid) - 1])
    return source_kparam_from_visible(
        dec,
        sg=sg,
        kslot=kslot,
        k_params=k_params,
...
def _display_records_for_star_arm(
    ...
    *,
    ...
    records = _display_k_records(source_records, lattice, symbol, label)
...
def _visible_k_values_for_gid(
    ...
    *,
    ...
    kslot = int(dec.little["little_irr_k"][int(gid) - 1])
    display_vector = str(_k_slot_info(dec, int(parent_sg), kslot).get("display_kvector") or "")
...
def _numeric_display_star_vectors(
    dec: SourceTables,
    parent_sg: int,
    gid: int,
    k_params: dict[str, str] | None,
    ...
    if parameter_values is None and visible is not None:
        source_kparam = _source_kparam_from_visible(dec, gid=gid, k_params=k_params)
        if source_kparam is not None:
            candidate = [
            ...
            represented = tuple(
                first_records[0][axis]
                + sum(
                    vector[axis] * candidate[param_index]
                    for param_index, vector in enumerate(first_records[1:])
                )
                for axis in range(3)
            )
            if all((represented[axis] - visible[axis]).denominator == 1 for axis in range(3)):
...
def _active_k_vectors_for_gid(
    ...
    lattice_slot = (lattice - 1) * 27 + kslot - 1
    dim = int(dec.little["little_k_dim"][lattice_slot])
    star = _numeric_display_star_vectors(dec, parent_sg, gid, k_params)
    if not star:
        star = [
            _compact_vector_text(substitute_k_vector(vector, k_params))
            for vector in _display_star_vectors(dec, parent_sg, kslot, dim)
        ]
    if not star:
        return []
...
def _isotropy_rows(
    ...
    *,
    ...
    for row_id in dec.isotropy_row_ids_for_old_irrep(old_id):
        ...
        # The public s value uses the native data_isotropy supercell basis,
...
def magnetic_opd_rows(
    ...
    *,
    ...
    table = magnetic_data().table
    out: list[dict[str, Any]] = []
    source_rows = magnetic_data().magnetic_isotropy_rows_for_irrep(int(old_id))
    canonical_row_id = int(source_rows[0].row_id) if source_rows else None
    source_rows = _selected_static_magnetic_rows(source_rows, selected_orderparam)
    for row in source_rows:
        ...
        iso_row = {
            ...
            "canonical": int(row.row_id) == canonical_row_id,
            "magnetic": True,
