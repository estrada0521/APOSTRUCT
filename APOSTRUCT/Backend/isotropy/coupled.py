...
from APOSTRUCT.Backend.source.tables import SourceTables
...
def _fixed_dimensions_for_point_actions(
    point_actions: tuple[tuple[int, ...], ...],
) -> tuple[int, int]:
    if not point_actions:
        raise ValueError("empty point-action group")

    polar_character = 0
    strain_character = 0
    for action in point_actions:
        trace = int(action[0]) + int(action[4]) + int(action[8])
        trace_squared_action = sum(
            int(action[3 * row + column]) * int(action[3 * column + row])
            for row in range(3)
            for column in range(3)
        )
        polar_character += trace
        strain_character += trace * trace + trace_squared_action

    order = len(point_actions)
    polar = Fraction(polar_character, order)
    strain = Fraction(strain_character, 2 * order)
    if polar.denominator != 1 or strain.denominator != 1:
        raise ValueError("point actions do not define integral fixed-space dimensions")
    return int(polar), int(strain)


def _fixed_polar_and_strain_dimensions(
    data: SourceData,
    operations: Sequence[tuple[int, int, int, int, int]],
) -> tuple[int, int]:
    point_actions = tuple(sorted({
        data.point_operation_matrix(int(operation[4])) for operation in operations
    }))
    return _fixed_dimensions_for_point_actions(point_actions)


def _ordinary_coupled_ferroic_properties(
    data: SourceData,
    parent_operations: Sequence[tuple[int, int, int, int, int]],
    child_operations: Sequence[tuple[int, int, int, int, int]],
    source_rows: Sequence[dict[str, Any]],
) -> list[str] | None:
    source_ferroic: list[dict[str, Any]] = []
    for row in source_rows:
        isotropy = row.get("isotropy") or {}
        ferroic = isotropy.get("ferroic")
        if bool(isotropy.get("dynamic")) or not isinstance(ferroic, dict):
            return None
        source_ferroic.append(ferroic)

    parent_polar, parent_strain = _fixed_polar_and_strain_dimensions(
        data, parent_operations
    )
    child_polar, child_strain = _fixed_polar_and_strain_dimensions(
        data, child_operations
    )
    properties: list[str] = []
    if child_polar > parent_polar:
        kind = (
            "proper"
            if any(
                int(row.get("raw") or 0) == 1
                and int(row.get("ferroelectric") or 0) == 1
                for row in source_ferroic
            )
            else "improper"
        )
        properties.append(f"{kind} ferroelectric")
    if child_strain > parent_strain:
        kind = (
            "proper"
            if any(
                int(row.get("raw") or 0) == 1
                and int(row.get("ferroelastic") or 0) == 1
                for row in source_ferroic
            )
            else "improper"
        )
        properties.append(f"{kind} ferroelastic")
    return properties
...
class _MagneticCoupledData:
    ...
    def orderparam_to_subgroup(
        ...
        return self._base._orderparam_to_subgroup_rows(  # noqa: SLF001
            gid=int(gid),
            kparam=kparam,
            orderparam=orderparam,
            row_count=row_count,
            candidates=self.generate_space_group_records(self._parent_sg),
            matrix_for_record=self.get_irreps_matrix_for_record,
        )
...
def _unique_static_factor_presentation(
    *,
    parent_sg: int,
    parent_operations: Sequence[tuple[int, int, int, int, int]],
    ordinary_subgroup: int,
    final_basis: Sequence[int],
    final_operations: Sequence[tuple[int, int, int, int, int]],
    source_rows: Sequence[dict[str, Any]],
    domain_tuple: Sequence[int],
    stored_presentations: Sequence[_StoredPresentation | None],
) -> tuple[tuple[int, ...], tuple[int, int, int, int]] | None:
    """Recover a unique stored factor embedding for the common subgroup.

    Intersecting a full-symmetry factor with a restrictive static factor can
    regenerate a symmetry-equivalent basis.  The stored factor row is the
    Source presentation authority when it already describes the exact common
    subgroup.  Ambiguous factor presentations keep the identified result.
    """

    if any(int(domain) != 0 for domain in domain_tuple):
        return None
    candidates: set[tuple[tuple[int, ...], tuple[int, int, int, int]]] = set()
    for source_row, stored in zip(source_rows, stored_presentations, strict=True):
        if stored is None or bool((source_row.get("isotropy") or {}).get("dynamic")):
            return None
        child, basis, origin, operations = stored
        if int(child) == int(ordinary_subgroup) and _same_subgroup(
            basis, operations, final_basis, final_operations
        ):
            candidates.add((basis, origin))
            continue
        if int(child) != int(parent_sg) or not _same_subgroup(
            basis, operations, _IDENTITY_BASIS, parent_operations
        ):
            return None
    if len(candidates) != 1:
        return None
...
def _operation_records(
    ...
    return data.kernel_operation_records(int(parent_sg), kernel_basis)
...
def _domain_actions(
    ...
    domain_subspaces: tuple[tuple[np.ndarray, int], ...] | None = None
    for operation in operations:
        ...
        if action is None:
            ...
            if domain_subspaces is None and any(
                _equation_key(dim, free, transformed) not in indices
                for transformed in transformed_domains
            ):
                domain_subspaces = _domain_subspaces(dim, free, domains)
            action = tuple(
                _domain_index(
                    dim,
                    free,
...
def _domain_index(
    ...
    domain_subspaces: Sequence[tuple[np.ndarray, int]] | None,
    indices: dict[tuple[int, tuple[float, ...]], int],
) -> int:
    ...
    if exact is not None:
        return exact
    if domain_subspaces is None:
        raise ValueError("missing domain subspaces for non-exact domain action")
    left = np.asarray(
    ...
    for index, (right, right_rank) in enumerate(domain_subspaces):
...
def _domain_subspaces(
    ...
    for domain in domains:
        ...
        out.append((right, int(np.linalg.matrix_rank(right, tol=subspace_tolerance))))
    return tuple(out)
...
def coupled_opd_rows(
    ...
    *,
    ...
    parent_operations = _operation_records(data, parent_sg, _IDENTITY_BASIS)
    ...
    for reversed_rows in product(*(rows for rows in reversed(ordered_slot_rows))):
        ...
        stored_presentations: list[_StoredPresentation | None] = []
        ...
        for domain_tuple in relative_domains:
            ...
            presentation = (
                None
                if magnetic_request
                else _unique_static_factor_presentation(
                    parent_sg=parent_sg,
                    parent_operations=parent_operations,
                    ordinary_subgroup=ordinary_subgroup,
                    final_basis=basis,
                    final_operations=subgroup_operations,
                    source_rows=source_rows,
                    domain_tuple=domain_tuple,
                    stored_presentations=stored_presentations,
                )
            )
            presentation_basis, presentation_origin = (
                presentation
            ...
            ordinary_ferroic_properties = (
                None
                if magnetic_request
                else _ordinary_coupled_ferroic_properties(
                    data,
                    parent_kernel_operations,
                    subgroup_operations,
                    source_rows,
                )
            )
            iso_row = {
                ...
                **(
                ...
                "ferroic_classified": (
                    True
                    if magnetic_request
                    else ordinary_ferroic_properties is not None
                ),
                "ferroic_properties": (
                    isotropy_catalog.magnetic_ferroic_properties(subgroup)
                    if magnetic_request
                    else ordinary_ferroic_properties or []
                ),
                "subduction": {
