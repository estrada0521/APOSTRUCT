...
from APOSTRUCT.Backend.isotropy.engine.solve_eqs import solve_eqs_mod_int_first
...
@dataclass(frozen=True)
class _InputOperationContext:
    point_ops: tuple[int, ...]
    slot_by_point_op: dict[int, int]
...
def id_subgroup_identify_with_generator_block(
    ...
    input_context = _input_operation_context(local_input_ops)
    ...
    for base_value_count in (3, 5, 7, 11):
        for candidate_sg in candidate_range:
            ...
            mappings = _candidate_generator_mappings(
                data,
                candidate_sg,
                input_context,
...
def _closure_accepts(
    data: SourceData,
    input_point_ops: Sequence[int],
    input_slot_by_point_op: dict[int, int],
    ...
    while changed:
        ...
        for left_index in range(mapped_count):
            for right_index in range(mapped_count):
                ...
                input_left_op = int(input_point_ops[mapped_input_slots[left_index] - 1])
                input_right_op = int(input_point_ops[mapped_input_slots[right_index] - 1])
                ...
                if existing_input_index != existing_candidate_index:
                    return False
    return len(mapped_input_slots) >= len(input_point_ops)
...
def candidate_generator_mappings(
    ...
    return _candidate_generator_mappings(
        data,
        candidate_subgroup,
        _input_operation_context(input_ops),
        candidate_generators,
    )


def _input_operation_context(
    input_ops: Sequence[OperationRecord],
) -> _InputOperationContext:
    point_ops = tuple(int(operation[4]) for operation in input_ops)
    return _InputOperationContext(
        point_ops=point_ops,
        slot_by_point_op={
            point_op: index + 1
            for index, point_op in enumerate(point_ops)
        },
    )


def _candidate_generator_mappings(
    data: SourceData,
    candidate_subgroup: int,
    input_context: _InputOperationContext,
    candidate_generators: Sequence[OperationRecord],
) -> tuple[GeneratorMapping, ...]:
    """Enumerate mappings against one already-indexed input operation set."""

    input_point_ops = input_context.point_ops
    input_slot_by_point_op = input_context.slot_by_point_op
    match_slots: list[tuple[int, ...]] = []
    for generator in candidate_generators:
        code = _point_code(data, int(generator[4]))
        slots = tuple(
            index + 1
            for index, point_op in enumerate(input_point_ops)
            if _point_code(data, point_op) == code
        )
    ...
    for ordinal in range(total):
        ...
        if not _closure_accepts(
            data,
            input_point_ops,
            input_slot_by_point_op,
            selected_slots,
            candidate_ops,
        ):
            continue
