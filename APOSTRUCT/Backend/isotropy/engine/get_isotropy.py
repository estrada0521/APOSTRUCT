...
from APOSTRUCT.Backend.isotropy.engine.dynamic_isotropy_file import DynamicIsotropyRow
from APOSTRUCT.Backend.isotropy.engine.id_subgroup import id_subgroup_identify_with_generator_block
...
from APOSTRUCT.Backend.isotropy.engine.orderparam import (
    eqs_to_orderparam,
    nice_orderparam,
    orderparam_check,
    orderparam_label,
...
from APOSTRUCT.Backend.isotropy.engine.numerics import ncmp, xrowop2
...
class PairProductCache(dict[tuple[Pair, Pair], Pair]):
    """Per-traversal operation-pair products and their ordinary factorization."""

    def __init__(self) -> None:
        super().__init__()
        self.factorized: _FactorizedPairProduct | None = None


class _FactorizedPairProduct:
    """Multiply ordinary operation pairs through their Source semidirect product."""
    ...
    def _fraction_index(self, values: tuple[Fraction, Fraction, Fraction]) -> int:
    ...
    def _space_product(self, left_space: int, right_space: int) -> Pair:
        key = int(left_space), int(right_space)
        cached = self._space_products.get(key)
        if cached is not None:
            return cached
        record = self.base._compose_operation_records(
            self.space_operations[key[0] - 1],
            self.space_operations[key[1] - 1],
            self.space_operations,
            lattice=self.lattice,
        )
        product = self.base._operation_pair_for_record(
            space_operations=self.space_operations,
            kernel_basis=self.kernel_basis,  # type: ignore[arg-type]
            kernel_fractions=self.kernel_fractions,
            record=record,
        )
        self._space_products[key] = product
        return product

    def _space_fraction(self, space_index: int, fraction_index: int) -> int:
        key = int(space_index), int(fraction_index)
        cached = self._space_fraction_actions.get(key)
        if cached is not None:
            return cached
        rotated = self.base._rotate_kernel_fraction_by_space_operation(
            self.kernel_fractions[key[1] - 1],
            self.space_operations[key[0] - 1],
            lattice=self.lattice,
        )
    ...
    def multiply(self, left: Pair, right: Pair) -> Pair:
        ...
        return space_index, self._fraction_index(
            tuple(
                left_values[axis] + correction_values[axis] + rotated_values[axis]
                for axis in range(3)
            )
        )


class _FactorizedMagneticPairProduct(_FactorizedPairProduct):
...
def _pair_product(
    ...
    *,
    ...
    if cached is not None:
        return cached
    factorized = getattr(cache, "factorized", None)
    product = (
        factorized.multiply(left, right)
        if factorized is not None
        else data.operation_pair_multiply(
...
def _projective_orbit_pairs(
    ...
    *,
    ...
    operation_cache: OperationCache,
    stride_matrix_cache: StrideMatrixCache,
) -> tuple[tuple[Pair, ...], dict[int, int]]:
    """Return first Source actions modulo an exact global sign.

    ``D`` and ``-D`` act on an OPD row space identically.  They must remain
    distinct for fixed-vector tests, but orbit canonicalization only consumes
    the row subspace, so it can retain the first action in Source order.
    """

    representatives: list[Pair] = []
    representative_by_signed_matrix: dict[tuple[float, ...], int] = {}
    representative_by_stride_id: dict[int, int] = {}
    for pair in _all_operation_pairs(len(generated_space), len(kernel_fractions)):
        ...
        first_nonzero = next((value for value in stride_matrix if value != 0.0), 0.0)
        signed_matrix = (
            stride_matrix
            if first_nonzero >= 0.0
            else tuple(-value for value in stride_matrix)
        )
        stride_id = id(stride_matrix)
        representative_id = representative_by_signed_matrix.get(signed_matrix)
        if representative_id is None:
            representative_id = stride_id
            representative_by_signed_matrix[signed_matrix] = representative_id
            representatives.append(pair)
        representative_by_stride_id[stride_id] = representative_id
    return tuple(representatives), representative_by_stride_id
...
def _stabilizer_pair_flags(
    ...
    *,
    ...
    for pair in pairs:
        ...
        flags[pair] = -1 if orderparam_check(
            int(dim),
            int(free),
            stride_matrix,
            orderparam,
            nonzero_columns=_stride_nonzeros(int(dim), stride_matrix, stride_nonzero_cache),
        ) else 0
    return flags
    ...
    *,
    ...
    for pair in pairs:
        ...
        if preserves is None:
            preserves = bool(orderparam_check(
                int(dim),
                int(free),
                stride_matrix,
                orderparam,
...
def _fixed_space_intersection(
    dim: int,
    free: int,
    orderparam: Sequence[float],
    irrep_matrix: Sequence[float],
    nonzero_cache: StrideNonzeroCache,
) -> tuple[int, tuple[float, ...], tuple[int, tuple[float, ...]]] | None:
    """Intersect one OPD with an operation's fixed space in OPD coordinates."""
    ...
    stride = 50
    coefficients = [0.0] * max(0, (int(free) - 1) * stride + int(dim))
    nonzero = False
    for component in range(int(dim)):
        for source_row in range(int(free)):
            index = source_row * 48 + component
            value = float(transformed[index]) - float(orderparam[index])
            nonzero = nonzero or ncmp(value)
            coefficients[component + (int(free) - source_row - 1) * stride] = value
    if not nonzero:
        return None

    reduced = xrowop2(coefficients, int(dim), int(free), stride=stride)
    rank = int(dim)
    while rank > 0 and not any(
        ncmp(float(reduced[(rank - 1) + column * stride]))
        for column in range(int(free))
    ):
        rank -= 1
    if rank <= 0 or rank >= int(free):
        return None

    next_free, coefficient_basis = _orderparam_from_equations(
        int(free), rank, reduced, reduce_first=False
    )
    raw = [0.0] * (int(next_free) * 48)
    for row in range(int(next_free)):
        for component in range(int(dim)):
            raw[row * 48 + component] = sum(
                float(coefficient_basis[row * 48 + source_row])
                * float(orderparam[source_row * 48 + component])
                for source_row in range(int(free))
            )
    eq_count, equations, key = _orderparam_equation_data(int(dim), int(next_free), raw)
    canonical_free, canonical = _orderparam_from_equations(
        int(dim), eq_count, equations, reduce_first=False
    )
    if canonical_free != next_free:
        raise ValueError("fixed-space intersection changed free dimension")
    return int(next_free), tuple(canonical), key
...
def _orbit_keys_and_representative(
    ...
    *,
    ...
    stride_matrix_cache: StrideMatrixCache,
    stride_nonzero_cache: StrideNonzeroCache,
    orbit_pairs: Sequence[Pair] | None = None,
    representative_by_stride_id: dict[int, int] | None = None,
    ...
    pairs = orbit_pairs or _all_operation_pairs(len(generated_space), len(kernel_fractions))
    for pair in pairs:
        stride_matrix = _stride_matrix_for_pair(
            data,
            gid=gid,
    ...
    if representative_by_stride_id is not None:
        for stride_id, representative_id in representative_by_stride_id.items():
            key = key_by_stride_id.get(representative_id)
            if key is not None:
                key_by_stride_id[stride_id] = key
    return keys, representative, key_by_stride_id
...
def _generate_dynamic_isotropy_rows(
    data: SourceData,
    *,
    traversal_data: SourceData | _MagneticDynamicData,
    gid: int,
    kparam: Sequence[int],
    point_occurrence: int | None,
    build_row: Callable[
        [int, Sequence[float], dict[int, int]], DynamicIsotropyRow | None
    ],
    configure_pair_product: Callable[
        [
    ...
    """Traverse dynamic OPD subspaces with path-specific Source adapters."""

    gid = int(gid)
    if point_occurrence is not None and int(point_occurrence) < 1:
    ...
    generated_space = traversal_data.generate_space_group_records(parent_sg)
    ...
    first_basis, _ = traversal_data.orderparam_to_subgroup(
        gid,
        tuple(int(value) for value in kparam),
        first,
        dim,
    ...
    first_keys, first_rep, _ = _orbit_keys_and_representative(
        traversal_data,  # type: ignore[arg-type]
        gid=gid,
        kparam=kparam,
        dim=dim,
        free=dim,
    ...
    orbit_pairs: tuple[Pair, ...] | None = None
    representative_by_stride_id: dict[int, int] | None = None

    while queue:
        free, node_orderparam, row_orderparam, pair_flags = queue.popleft()
        row = build_row(free, row_orderparam, counts) if point_occurrence is None or int(free) == 1 else None
        if row is not None:
            rows.append(row)
            if (
                point_occurrence is not None
                and int(row.free) == 1
                and counts.get(1) == int(point_occurrence)
            ):
                return tuple(rows)
        ...
        for pair in _candidate_pairs_with_parent_closure(
            ...
            reduced_intersection = _fixed_space_intersection(
                dim,
                free,
                node_orderparam,
                stride_matrix,
            ...
            if reduced_intersection is None:
                continue
            next_free, next_orderparam, key = reduced_intersection
            if key in seen:
                continue
            if orbit_pairs is None:
                orbit_pairs, representative_by_stride_id = _projective_orbit_pairs(
                    traversal_data,  # type: ignore[arg-type]
                    gid=gid,
                    kparam=kparam,
                    generated_space=generated_space,
            ...
            orbit_keys, representative, orbit_key_by_stride_id = _orbit_keys_and_representative(
                ...
                representative_by_stride_id=representative_by_stride_id,
            )
            if orbit_keys & seen:
                continue
...
def _configure_ordinary_pair_product(
    data: SourceData,
    generated_space: Sequence[tuple[int, int, int, int, int]],
    kernel_basis: Sequence[int],
    kernel_fractions: Sequence[tuple[int, int, int, int]],
    ...
    cache.factorized = _FactorizedPairProduct(
        data,
        space_operations=generated_space,
        kernel_basis=kernel_basis,
        kernel_fractions=kernel_fractions,
...
def generate_dynamic_isotropy_rows(
    ...
    *,
    ...
    """Generate dynamic ``*.iso`` rows in Source discovery order."""

    parent_sg = int(data.little["little_irr_space_group"][int(gid) - 1])
    ...
    return _generate_dynamic_isotropy_rows(
        data,
        traversal_data=data,
        gid=int(gid),
        kparam=kparam,
        point_occurrence=_point_occurrence,
        build_row=build_row,
        configure_pair_product=lambda operations, basis, fractions, lattice, cache: _configure_ordinary_pair_product(
            data, operations, basis, fractions, lattice, cache
        ),
    )


def _dynamic_point_isotropy_row(rows: Sequence[DynamicIsotropyRow], occurrence: int) -> DynamicIsotropyRow | None:
    label = orderparam_label(1, int(occurrence))
    return next(
...
def generate_dynamic_point_isotropy_row(
    ...
    *,
    ...
    return _dynamic_point_isotropy_row(rows, occurrence)
...
def generate_dynamic_isotropy_rows_magnetic(
    ...
    *,
    ...
    """Generate magnetic dynamic rows through the common OPD traversal."""
    ...
    def build_row(
        ...
        return _row_from_orderparam_magnetic(
            data,
            parent_sg=parent_sg,
            gid=gid,
            kparam=kparam,
    ...
    def configure_pair_product(
        ...
        basis: Sequence[int],
        fractions: Sequence[tuple[int, int, int, int]],
        lattice: int,
        _cache: PairProductCache,
    ) -> None:
    ...
    return _generate_dynamic_isotropy_rows(
        data,
        traversal_data=magnetic_data,
        gid=gid,
        kparam=kparam,
        point_occurrence=_point_occurrence,
        build_row=build_row,
        configure_pair_product=configure_pair_product,
    )
