...
from APOSTRUCT.Backend.exactmath import fraction_record3, fraction_row_multiply3
...
def _candidate_magnetic_generator_mappings_core(
    input_ops: Sequence[Sequence[int]],
    candidate_magnetic_group: int,
    candidate_generators: Sequence[Sequence[int]],
    ...
    for ordinal in range(total):
        ...
        accepted.append(
            MagneticGeneratorMapping(
                candidate_magnetic_group=int(candidate_magnetic_group),
...
def _candidate_magnetic_generator_mappings_cached(
    input_point_ops: tuple[int, ...],
    candidate_ops: tuple[int, ...],
) -> tuple[MagneticGeneratorMapping, ...]:
    """Cache the translation-independent point-group closure pattern."""

    point_records = tuple((0, 0, 0, 1, int(value)) for value in input_point_ops)
    candidate_records = tuple((0, 0, 0, 1, int(value)) for value in candidate_ops)
    return _candidate_magnetic_generator_mappings_core(
        point_records,
        0,
        candidate_records,
    )
    ...
    *,
    ...
    for slot, (active, local) in enumerate(
        ...
        if tuple(value % 1 for value in canonical_translation) != _record_translation_mod1(
            ...
            return ()
        translation_record = fraction_record3(canonical_translation)
        out.append(
    ...
    for base_value_count in (3, 5, 7, 11):
        for candidate_scan_ordinal, candidate in enumerate(candidates, start=1):
            ...
            if lattice_constraint is not None and candidate_lattice != lattice_constraint:
                continue
            mappings = _candidate_magnetic_generator_mappings_cached(
                tuple(int(record[4]) for record in local_input_ops),
                tuple(int(record[4]) for record in candidate.generators),
            )
            if not mappings:
                continue
