    ...
    *,
    ...
    for source_col in range(cols):
        ...
        if source_col == cols - 1 or active_rows == eq_stride:
            xrowop2(out, active_rows, cols, stride=eq_stride, in_place=True)
...
def orderparam_check(
    ...
    *,
    matrix_stride: int = 48,
    op_stride: int = 48,
    nonzero_columns: Sequence[Sequence[tuple[int, float]]] | None = None,
) -> int:
    ...
    matrix_length = len(irrep_matrix)
    orderparam_length = len(orderparam)
    for row in range(row_count):
        orderparam_offset = row * op_stride
        for col in range(dim):
            transformed = 0.0
            if nonzero_columns is None:
                for source_row in range(dim):
                    matrix_index = source_row * matrix_stride + col
                    orderparam_index = orderparam_offset + source_row
                    matrix_value = (
                        float(irrep_matrix[matrix_index]) if matrix_index < matrix_length else 0.0
                    )
                    orderparam_value = (
                        float(orderparam[orderparam_index]) if orderparam_index < orderparam_length else 0.0
                    )
                    transformed += matrix_value * orderparam_value
            else:
                for source_row, matrix_value in nonzero_columns[col]:
                    orderparam_index = orderparam_offset + source_row
                    orderparam_value = (
                        float(orderparam[orderparam_index]) if orderparam_index < orderparam_length else 0.0
                    )
                    transformed += matrix_value * orderparam_value
            expected_index = orderparam_offset + col
            expected = float(orderparam[expected_index]) if expected_index < orderparam_length else 0.0
            if ncmp(transformed - expected):
                return 0
    return 1
