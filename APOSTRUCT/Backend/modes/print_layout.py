...
def _mode_site_irrep_labels(
    ...
    *,
    ...
    try:
        ...
        raise ValueError(
            "cannot resolve site-irrep labels from the mode block: "
            f"mode_count={mode_count}, summaries={len(summaries)}, "
            f"site_pg={(trace.get('wyckoff') or {}).get('site_pg')!r}"
        ) from exc
    totals: dict[str, int] = {}
    ...
    for summary, base, dimension in zip(summaries, bases, dimensions, strict=True):
        ...
        if dimension <= 0 or count <= 0:
            raise ValueError(
                "site-irrep label layout has a nonpositive dimension or mode count"
            )
        display_dimension = dimension
        source_row_count = int(summary.get("source_row_count") or 0)
        opd_groups = summary.get("opd_groups") or []
        opd_row_count = sum(
            len(group.get("rows") or [])
            for group in opd_groups
            if isinstance(group, dict)
        )
        opd_component_eligible = (
            (
        ...
        selected_opd_rows_are_explicit = bool(opd_groups) and all(
            isinstance(group, dict)
            and isinstance(group.get("rows"), (list, tuple))
            and bool(group["rows"])
            and all(
                isinstance(row, (list, tuple)) and bool(row)
        ...
        if (
            ...
            and (
                ...
                or (count % dimension != 0 and selected_opd_rows_are_explicit)
            )
        ):
        ...
        if count % display_dimension:
            if count == 1:
                ...
                raise ValueError(
                    "site-irrep label dimension does not divide the emitted mode count: "
                    f"count={count}, dimension={display_dimension}, base={base!r}"
                )
    ...
    if len(labels) != mode_count:
        raise ValueError(
            "site-irrep label count does not match the emitted mode count: "
            f"labels={len(labels)}, modes={mode_count}"
        )
    return labels
    ...
    *,
    ...
    layout = [
        ...
        for component, print_component in enumerate(
            _summary_source_print_components(summary)
        )
    ]
    return layout if len(layout) == mode_count else None
