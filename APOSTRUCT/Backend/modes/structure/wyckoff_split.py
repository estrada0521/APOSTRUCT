...
from APOSTRUCT.Backend.isotropy.engine.lattice import (
    _reduce_fraction_vector,
    get_new_fractionals,
)
from APOSTRUCT.Backend.source.tables import SourceTables, WyckoffRow
...
def _origin_record(origin: list[float] | tuple[float, ...] | None) -> FractionRecord:
    if origin is None:
        return (0, 0, 0, 1)
    if len(origin) == 4:
        return _reduce_fraction_vector(tuple(int(value) for value in origin))  # type: ignore[arg-type]
...
def transform_wyckoff_branches_to_child(
    ...
    for kernel_fraction in get_new_fractionals(basis):
...
def _affine_subset_of_child(
    ...
    child_key = tuple(child_params)
    child_rank = _rank(child_key)
    zero = Fraction(0)
    basis = next((column for column in child_key if any(column)), (zero, zero, zero))
    normal = next(
        (candidate for column in child_key if any(candidate := _cross3(basis, column))),
        (zero, zero, zero),
    )

    def in_child_span(vector: tuple[Fraction, Fraction, Fraction]) -> bool:
        if child_rank == 0:
            return not any(vector)
        if child_rank == 1:
            return not any(_cross3(basis, vector))
        if child_rank == 2:
            return sum(normal[axis] * vector[axis] for axis in range(3)) == 0
        return True

    if any(not in_child_span(param) for param in params):
        return False
...
def _solve_child_parameter_column(
    ...
    *,
    ...
    for offset in offsets:
        ...
        augmented, pivot_columns = fraction_rref(
...
def _get_new_wyckoff_child_rows_formula15_provenance_cached(
    ...
    kernel_fractions = get_new_fractionals(tuple(int(value) for value in subgroup_basis[:9]))
