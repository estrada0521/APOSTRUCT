...
@lru_cache(maxsize=4096)
def _formula_constraint_shift_index(
    constraints: tuple[tuple[Fraction, Fraction, Fraction], ...],
    ranges: tuple[tuple[int, int], tuple[int, int], tuple[int, int]],
) -> Mapping[tuple[Fraction, ...], tuple[int, int, int]]:
    out: dict[tuple[Fraction, ...], tuple[int, int, int]] = {}
    for shift in itertools.product(
        *(range(start, stop) for start, stop in ranges)
    ):
        value = tuple(
            sum(row[axis] * shift[axis] for axis in range(3))
            for row in constraints
        )
        out.setdefault(value, shift)
...
def _formula_solution(
    ...
    constraints = tuple(
        transform[row]
        for row in range(3)
        if all(reduced[row][column] == 0 for column in range(3))
    )
    ranges = tuple(
        (
    ...
    if constraints:
        delta = tuple(point[axis] - formula[0][axis] for axis in range(3))
        required = tuple(
            -sum(row[axis] * delta[axis] for axis in range(3))
            for row in constraints
        )
        shift = _formula_constraint_shift_index(constraints, ranges).get(required)
        if shift is None:
            return None
    else:
        shift = tuple(start for start, _stop in ranges)
    target = tuple(
    ...
    for row, column in enumerate(pivots):
        values[column] = sum(
            transform[row][axis] * target[axis] for axis in range(3)
        )
