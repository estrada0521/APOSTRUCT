...
def _centering_translations_for_space_group(
    child_sg: int,
) -> tuple[ExactPoint, ...] | None:
    try:
        space_group = gemmi.find_spacegroup_by_number(child_sg)
...
def _centering_translations(child_sg: int) -> tuple[ExactPoint, ...] | None:
    if (
        ...
        return None
    return _centering_translations_for_space_group(int(child_sg))
