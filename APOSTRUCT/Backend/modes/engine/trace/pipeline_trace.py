...
Production authority for type-1 parametric displacive littles
-------------------------------------------------------------
When ``resolve_display_projection`` adopts the Source DISPLAY DISTORTION plan
for a little (type-1 ∧ k_params ∧ vector_setting=1), that plan is the sole
production definition of the mode vectors for the little. The recordwise
Fortran bridge packing below is not consulted on that path; it remains the
production path only when display declines (or is ineligible), and a RE/test
probe may force-decline display to compare the two packings.
"""
...
from APOSTRUCT.Backend.modes.engine.project.mode_forms import (
    _add_mode_vectors,
...
from APOSTRUCT.Backend.modes.engine.project.main_bridge import (
...
from APOSTRUCT.Backend.modes.engine.project import display_distortion as _display_distortion
from APOSTRUCT.Backend.modes.engine.project.display_distortion import (
    DisplayProjectionBlock,
)
from APOSTRUCT.Backend.modes.engine.project.mode_counts import little_records_for_k, mode_totals
from APOSTRUCT.Backend.modes.engine.project.entry import (
    project_entry_trace,
    project_local408_trace,
    site_get_irreps_trace,
)
from APOSTRUCT.Backend.modes.engine.trace.project_sources import (
...
def pipeline_trace(
    ...
    *,
    ...
    _display_projection_cache: dict[bytes, tuple[DisplayProjectionBlock, ...]] | None = None,
    ...
    for little in littles:
        ...
        if occurrence_atom_basis is not None or baseline_atom_basis is not None:
            ...
            # Occurrence-cell materialization is only useful when the Source
            # display plan can adopt. Skip it for non-type-1 / non-parametric /
            # non-displacive vector settings so the bridge path does not pay for
            # a layout that build_candidate_projection would immediately decline.
            display_candidate_eligible = (
                little.irrep_type == 1
                and bool(case.k_params)
                and int(vector_setting) == 1
                and bool(direct_direction_rows)
            )
            candidate_layout = None
            if occurrence_atom_basis is not None and display_candidate_eligible:
                try:
            ...
            def build_candidate_projection(
                ...
                if not display_candidate_eligible:
                    return None
                _fractionals, candidate_records, _displayed, candidate_order = layout
                return _display_distortion.source_display_projection_plan(
                    decoder,
                    case,
                    gid=int(little.gid),
                    ...
                    _cache=_display_projection_cache,
                )
            ...
            layout, direct_projection, projection_adopted = (
                _display_distortion.resolve_display_projection(
                    candidate_layout,
            ...
            # Display-adopted littles never enter the recordwise bridge loop.
            # (Adopt is all-or-nothing per little: every prep has a block.)
            if projection_adopted:
                direct_groups = [
                    {
                        "index": int(index),
                ...
                for block in direct_projection:
                        ...
                        mode_block_summaries_by_gid[little.gid].append(
                            {
                                ...
                                "emitted_mode_count": int(block.project_count),
                                "project_count": int(block.project_count),
                                "bridge_project_count": int(
                                    block.bridge_project_counts[family.family]
                                ),
                                "vector_dim": int(block.vector_dim),
                ...
                def recordwise_project_vector_modes() -> list[list[list[float]]]:
                    ...
                    for source in source_iter:
                        ...
                        if (
                            ...
                            print_group_count = source_row_count
            ...
            if little.irrep_type == 3 and case.k_params:
                summaries = mode_block_summaries_by_gid[little.gid]
                if summaries:
                    # Source summaries are independent print blocks; a four-mode
                    # pattern must not be assembled across adjacent summaries.
                    mode_vectors_by_gid[little.gid] = (
                        _type3_parametric_component_print_order_by_block(
                            mode_vectors_by_gid[little.gid],
                            [int(summary["flat_mode_count"]) for summary in summaries],
                        )
                    )
                else:
                    mode_vectors_by_gid[little.gid] = (
