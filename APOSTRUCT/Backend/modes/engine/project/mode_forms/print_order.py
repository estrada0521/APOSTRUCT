...
def _type3_parametric_component_print_order_by_block(
    modes: list[list[list[float]]],
    block_mode_counts: Sequence[int],
) -> list[list[list[float]]]:
    """Apply the type-3 print order within each Source mode block."""

    counts = tuple(int(count) for count in block_mode_counts)
    if any(count <= 0 for count in counts) or sum(counts) != len(modes):
        raise ValueError(
            "type-3 mode block counts do not partition the emitted modes"
        )
    out: list[list[list[float]]] = []
    offset = 0
    for count in counts:
