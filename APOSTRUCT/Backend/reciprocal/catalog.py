...
from APOSTRUCT.Backend.exactmath import fraction_matrix_inverse3, fraction_rref
...
def _rref_k_directions(
    ...
    reduced, pivots = fraction_rref(work)
    basis = tuple(tuple(row) for row in reduced[:len(pivots)])
    return basis, pivots  # type: ignore[return-value]
