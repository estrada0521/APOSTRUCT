...
class SourceTables:
    ...
    source of truth. The packaged runtime verifies those files against its
    manifest and loads individual decoded sections only when requested.
    """
    ...
    def international_setting_records_for_space_group(self, sg: int) -> list[dict[str, Any]]:
        """Return the non-legacy International-setting choices stored by Source."""
        ...
        return [
            ...
            if (record := self.inter_setting_record(setting_id))["version"] != 2
        ]

    def international_setting_record(self, sg: int, setting_id: int | None) -> dict[str, Any]:
        """Resolve one non-legacy International setting, defaulting to the SG preference."""
    ...
    def match_inter_wyckoff_site(
        ...
        for row in self.wyckoff_rows(sg):
            ...
            source_setting_id = int(
                self.space["ispace_inter_choice"][int(sg) - 1]
            )
            display_source_params = (
                self._solve_inter_wyckoff_params_source_branch(
                    sg,
                    row,
                    source_fract,
                    tol=tol,
                )
                if int(setting_id) == source_setting_id
                else None
            )
    ...
    def wyckoff_subduction_pairs_for_little_gid(
        ...
        This is the APOSTRUCT backend equivalent of the upstream SMODES
