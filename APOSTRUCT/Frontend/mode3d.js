...
export class ModeStructureViewer {
  constructor({
    ...
    loadElementStyles("./vesta_elements.csv").then(() => {
      this.updateElementSwatches();
      this.updateScene();
    });
    this.bindControls();
    this.bindViewCube();
  ...
  setAmplitudes(rows) {
    const available = new Set(this.modes.map(mode => mode.id));
    for (const row of Array.isArray(rows) ? rows : []) {
      const definitionId = String(row?.definition_id || "");
      if (!available.has(definitionId)) {
        throw new Error(`Mode definition ${definitionId || "?"} is unavailable in this viewer`);
      }
      const value = number(row?.value, Number.NaN);
      if (!Number.isFinite(value)) {
        throw new Error(`Mode amplitude for ${definitionId} is not numeric`);
      }
      this.amplitudes.set(definitionId, value);
    }
    this.renderModeControls();
    this.updateScene();
  }
  ...
  renderModeControls() {
    ...
    const renderModes = modes => modes.map(mode => {
      ...
      const limit = Math.max(this.modeAmplitudeLimit(mode), Math.abs(value));
    ...
    const atomVisibility = childSites.length ? `<fieldset class="mode-atom-visibility">
      ...
      <div class="mode-atom-tree">${[...parentSitesByElement.entries()].map(([element, parentSites]) => `<details class="mode-atom-branch mode-element-branch">
        <summary><label class="mode-atom-toggle mode-element-toggle">
          ...
          <span class="mode-element-swatch" data-mode-element-swatch="${this.escape(element)}" style="background-color:${this.elementColorCss(element)}" aria-hidden="true"></span>
        </label></summary>
  ...
  elementColorCss(element) {
    return colorCss(softenedMaterialColor(atomColor({ element })));
  }

  updateElementSwatches() {
    this.controlsNode?.querySelectorAll("[data-mode-element-swatch]").forEach(swatch => {
      swatch.style.backgroundColor = this.elementColorCss(swatch.dataset.modeElementSwatch);
    });
  }
  ...
  updateScene() {
    ...
    if (parentCell) {
      ...
      const parentOffset = Array.isArray(viewerOrigin)
        ...
        : new THREE.Vector3();
      // Strain the parent lattice before applying the subgroup
