"""Argument parser for the APOSTRUCT command line."""
...
def _version_text() -> str:
    try:
        package_version = version("APOSTRUCT")
    ...
    return f"APOSTRUCT {package_version}{suffix}"
...
def parser() -> argparse.ArgumentParser:
    root = argparse.ArgumentParser(
        ...
        epilog="""workflow:
  apo settings --sg 14
  apo directions --parent-sg 221 --subgroup-sg 140 --basis="...;...;..."
  apo info structure.cif
  apo kpoints structure.cif
  apo irreps structure.cif --k R --displacive O
  apo opds structure.cif --k R --irrep R4- --displacive O
  apo modes structure.cif --k R --irrep R4- --displacive O --opd P1
  apo invariants structure.cif --k R --irrep R4- --displacive O --opd P1
  apo invariants --from-modes modes.json --secondary 2
  apo combine-modes modes.json --weight displacive-1=1
...
Use --full-state only when complete pipeline provenance is required.""",
    ...
    root.add_argument("--version", action=_VersionAction)
    ...
    directions = _command_parser(
        ...
        example=(
            "apo directions --parent-sg 221 --subgroup-sg 140 "
    ...
    directions.add_argument(
        "--parent-setting",
        type=int,
        help="International setting ID from apo settings --sg PARENT_SG",
    )
    ...
    directions.add_argument(
        ...
        help="three subgroup basis rows in parent coordinates, separated by semicolons",
    )
    directions.add_argument(
        "--origin",
        default="0,0,0",
    ...
    info = _command_parser(
        ...
        example="apo info --sg 221 --wyckoff 1a 1b 3c",
    )
    ...
    kpoints = _command_parser(
        ...
        example="apo kpoints structure.cif",
    )
    ...
    irreps = _command_parser(
        ...
        example="apo irreps structure.cif --k DT b=1/3 --displacive O",
    )
    ...
    modes = _command_parser(
        ...
        example=(
            "apo modes structure.cif --k R --irrep R4- "
            "--displacive O --opd P1"
        ),
    )
    ...
    invariants = _command_parser(
        ...
        example=(
            "apo invariants structure.cif --k R --irrep R4- "
    ...
    serve = _command_parser(
        ...
        example="apo serve --host 127.0.0.1 --port 8300",
    )
    serve.add_argument("--host", default="127.0.0.1")
    serve.add_argument("--port", type=int, default=8300)
    serve.add_argument("--open-browser", action="store_true")
    show = _command_parser(
        commands,
        "show",
        help_text="show a JSON case in a running graphical interface",
        description=(
            "Send one existing JSON case to a running APOSTRUCT server.\n"
            "The GUI opens the deepest stage described by the case. Repeatable "
            "--amplitude values initialize mode sliders when the case includes opd."
        ),
        example=(
            "apo show --case analysis.json "
            "--server http://127.0.0.1:8300"
        ),
    )
    show.add_argument("--case", type=Path, required=True, help="JSON case path or - for stdin")
    show.add_argument(
        "--server",
        default="http://127.0.0.1:8300",
        help="running APOSTRUCT server URL",
    )
    show.add_argument(
        "--amplitude",
        action="append",
        default=[],
        metavar="DEFINITION_ID=VALUE",
        help="repeatable initial mode-slider amplitude for an OPD case",
    )
