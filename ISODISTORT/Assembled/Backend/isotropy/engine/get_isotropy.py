...
from ISODISTORT.Assembled.Backend.isotropy.engine.id_subgroup import id_subgroup_identify_with_generator_block
...
from ISODISTORT.Assembled.Backend.isotropy.engine.orderparam import (
    eqs_to_orderparam,
    nice_orderparam,
    orderparam_add_eqs,
    orderparam_check,
    ...
    def operation_pair_multiply(
        ...
        *,
        ...
        cache_key = (
            ...
            self._base._sequence_key(kernel_fractions, cache_name="_mag_kernel_fraction_sequence_key_cache"),
            int(lattice),
            (int(left[0]), int(left[1])),
            (int(right[0]), int(right[1])),
        )
        cached = self._operation_pair_multiply_cache.get(cache_key)
        if cached is not None:
            return cached

        left_record = self._base._vadd_fraction_operation(
            kernel_fractions[int(left[1]) - 1],
            space_operations[int(left[0]) - 1],
        )
        right_record = self._base._vadd_fraction_operation(
            kernel_fractions[int(right[1]) - 1],
            space_operations[int(right[0]) - 1],
        )
        product_record = self._base._compose_magnetic_operation_records(
            left_record,
            right_record,
            sg=self._ordinary_sg,
        )
        result = self._base._operation_pair_for_record(
            space_operations=space_operations,
            kernel_basis=kernel_basis,
            kernel_fractions=kernel_fractions,
            record=product_record,
        )
        self._operation_pair_multiply_cache[cache_key] = result
        return result
