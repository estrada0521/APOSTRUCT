...
from ISODISTORT.Assembled.Backend.modes.engine.tensor import _cartesian_point_matrix
from .source import (
...
def fixed_gradient_invariants(
    space_group: int,
    irreps: Sequence[str],
    *,
    degree: int,
    gradient_order: int,
) -> tuple[tuple[Polynomial, ...], tuple[GradientVariable, ...], int]:
    ...
    return basis, variables, len(matrices)


def parametric_gradient_invariants(
    space_group: int,
    irreps: Sequence[str],
    k_parameters: Sequence[Sequence[float | Fraction | int]],
    *,
