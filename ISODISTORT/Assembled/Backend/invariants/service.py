...
from ISODISTORT.Assembled.Backend.isotropy.catalog import (
    _basis_text,
...
from ISODISTORT.Assembled.Backend.reciprocal.catalog import kpoints as reciprocal_kpoints
from ISODISTORT.Assembled.Backend.source.magnetic import data as magnetic_source_data
...
class InvariantService:
    ...
    def catalog(self, space_group: object) -> dict[str, Any]:
        ...
        for source_k in reciprocal["kpoints"]:
            ...
            expression, source_parameter_names = _canonical_k_expression(
                str(source_k["isodistort_kvector"])
            )
    ...
    def _dynamic_isotropy(
        ...
        if int(factor["old_id"]) > 0:
            catalog = (
                catalog_magnetic_opd_rows
                if factor["magnetic"]
                else catalog_opd_rows
            )
            rows = catalog(
                int(factor["old_id"]),
                parent_sg=int(sg),
    ...
    def _ordinary_action(
        ...
        for factor, direction, domain in zip(factors, directions, domains, strict=True):
            options = {
                item["label"]: item["domain_count"]
            ...
            if domain > options[direction]:
                raise ValueError(
                    f"domain {domain} exceeds the upper bound {options[direction]} for {direction}"
                )
