...
def project_counts_for_old(
    ...
    """Return project counts keyed by site PG irrep without label lookup."""

    rows = [
        item for item in decoder.wyckoff_subductions_for_old(sg, old_id)
        if item.wyckoff.row_id == row_id
    ]
    if len(rows) != 1:
        raise KeyError(f"expected one subduction for SG{sg} row_id={row_id} old={old_id}")
    divisor = decoder.TYPE_DIVISOR[irrep_type]
    out: dict[int, int] = {}
    for pg_irrep, frequency in rows[0].pairs:
        ...
        if numerator % divisor:
            raise ValueError(
                f"nonintegral project count: SG{sg} old={old_id} "
