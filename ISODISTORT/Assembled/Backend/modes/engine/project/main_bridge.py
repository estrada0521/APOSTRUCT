...
def dense_type3_parametric_source(term_count: int) -> list[float] | None:
    term_sets: tuple[tuple[int, ...], tuple[int, ...]] | None = None
    if term_count == 4:
        term_sets = ((0, 2), (1, 3))
    elif term_count == 8:
        term_sets = ((0, 5), (2, 7))
    if term_sets is None:
        return None
    dense = [0.0] * 512
    offset = 16 - term_count
    for row_index, terms in enumerate(term_sets):
        for term_index in terms:
            dense[row_index * 48 + offset + term_index] = 1.0
    return dense
