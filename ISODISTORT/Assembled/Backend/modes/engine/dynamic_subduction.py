    ...
    *,
    ...
    for row in rows:
        ...
        for source_kparam, occurrences in classes:
            if source_kparam == represented_source_kparam:
                continue
            try:
                exact_aliases = [
                    alias
...
@dataclass(frozen=True)
class KvecStandardType1ProjectCase:
    """One faithful type-1 ``project_`` evaluation at a concrete k value."""

    kparam_record: tuple[int, int, int, int]
    k_parameters: tuple[Fraction, ...]
    operation_records: tuple[OperationRecord, ...]
    operation_matrices: tuple[tuple[tuple[complex, ...], ...], ...]
    site_ops: tuple[int, ...]
    selected_indices: tuple[int, ...]
    real_blocks: tuple[tuple[tuple[complex, ...], ...], ...]


@dataclass(frozen=True)
class KvecStandardType1ProjectSurface:
    ...
    provenance: KvecStandardProvenance
    source_case: KvecStandardType1ProjectCase
    standard_case: KvecStandardType1ProjectCase
...
def _source_kparam_for_vector(
    ...
    return _source_kparam_for_vector_cached(
        decoder,
        int(sg),
        int(gid),
...
def _source_kparam_for_vector_cached(
    decoder: Any,
    sg: int,
    gid: int,
...
def _little_k_vectors_by_gid_kparam(
    ...
    values = tuple(kparam)
    return _little_k_vectors_by_gid_kparam_cached(decoder, int(gid), values)


@lru_cache(maxsize=8192)
def _little_k_vectors_by_gid_kparam_cached(
    decoder: Any,
    gid: int,
...
@lru_cache(maxsize=4096)
def _first_little_k_vector_by_gid_kparam_cached(
    decoder: Any,
    gid: int,
    values: tuple[Fraction | int, ...],
...
def _kvec_standard_selection(
    ...
    original_vector = _first_little_k_vector_by_gid_kparam_cached(
        decoder,
...
def kvec_standard_provenance(
    ...
    """Expose Source-only standardization provenance to Assembled callers."""
...
def _k_parameters_for_record(
    ...
    if denominator == 0:
        raise ValueError(f"invalid k parameter record: {record!r}")
    sg = int(decoder.iso.little["little_irr_space_group"][int(gid) - 1])
    kslot = int(decoder.iso.little["little_irr_k"][int(gid) - 1])
    lattice = int(decoder.iso.space["ispace_lattice"][sg - 1])
    dimension = int(decoder.iso.little["little_k_dim"][(lattice - 1) * 27 + kslot - 1])
    return tuple(Fraction(int(record[index]), denominator) for index in range(max(0, dimension)))


def _complex_matrix_tuple(matrix: np.ndarray) -> tuple[tuple[complex, ...], ...]:
    values = np.array(matrix, dtype=complex, copy=True)
    values.real[np.abs(values.real) < 1e-12] = 0.0
    values.imag[np.abs(values.imag) < 1e-12] = 0.0
    return tuple(tuple(complex(value) for value in row) for row in values)


def _type1_project_case(
    decoder: Any,
    *,
    template_case: Case,
    gid: int,
    ...
    project_count: int,
    kparam_record: tuple[int, int, int, int],
) -> KvecStandardType1ProjectCase:
    k_parameters = _k_parameters_for_record(decoder, int(gid), kparam_record)
    case = replace(template_case, k_params=k_parameters)
    rows = [row for row in decoder.wyckoff_rows(int(case.sg)) if row.label == str(case.wyckoff)]
    if len(rows) != 1:
        raise KeyError(f"expected one SG{case.sg} Wyckoff {case.wyckoff}, got {len(rows)}")
    little = decoder.little_record_by_gid(int(gid))
    records = tuple(
        tuple(int(value) for value in record)
        for record in decoder.project_parent_operation_records_for_little_row(
            int(case.sg), little, rows[0], case.site_params
        )
    )
    selection = decoder.project_select_real_blocks_type1(
        int(gid),
        int(rows[0].site_pg),
        int(target_vector_rep),
        records,
        int(project_count),
        case,
    )
    matrices = tuple(
        _complex_matrix_tuple(
            decoder.little_phase_matrix_by_gid_record_for_case(int(gid), record, case)
        )
        for record in records
    )
    return KvecStandardType1ProjectCase(
        kparam_record=tuple(int(value) for value in kparam_record),
        k_parameters=k_parameters,
        operation_records=records,
        operation_matrices=matrices,
        site_ops=tuple(int(value) for value in selection.site_ops),
        selected_indices=tuple(int(value) for value in selection.selected_indices),
        real_blocks=tuple(
            tuple(tuple(complex(value) for value in row) for row in family)
            for family in np.asarray(selection.real_blocks, dtype=complex)
        ),
    )
...
def kvec_standard_type1_project_surface(
    ...
    project_count: int,
    source_kparam: tuple[int, int, int, int],
) -> KvecStandardType1ProjectSurface:
    """Expose paired faithful type-1 project cases without presentation policy."""

    gid = int(representative_gid)
    little = decoder.little_record_by_gid(gid)
    if int(little.irrep_type) != 1:
        raise ValueError(f"gid={gid} is type {little.irrep_type}, not type 1")
    provenance = kvec_standard_provenance(
        decoder,
        int(template_case.sg),
        gid,
        tuple(int(value) for value in source_kparam),
    )
    source_case = _type1_project_case(
        decoder,
        template_case=template_case,
        gid=gid,
        target_vector_rep=int(target_vector_rep),
        project_count=int(project_count),
        kparam_record=provenance.source_kparam,
    )
    standard_case = _type1_project_case(
        decoder,
        template_case=template_case,
        gid=gid,
        target_vector_rep=int(target_vector_rep),
        project_count=int(project_count),
        kparam_record=provenance.standard_kparam,
    )
    selected_record = provenance.selected_operation_record
    if selected_record is None:
        raise ValueError(
            f"kvec_standard selection has no full operation record for SG{template_case.sg} gid={gid}"
        )
    source_runtime = replace(template_case, k_params=source_case.k_parameters)
    standard_runtime = replace(template_case, k_params=standard_case.k_parameters)
    source_matrix = decoder.little_phase_matrix_by_gid_record_for_case(
        gid, selected_record, source_runtime
    )
    standard_matrix = decoder.little_phase_matrix_by_gid_record_for_case(
        gid, selected_record, standard_runtime
    )
    try:
        quotient = np.linalg.solve(standard_matrix, source_matrix)
    except np.linalg.LinAlgError as exc:
        raise ValueError(
            f"singular selected type-1 operation matrix for SG{template_case.sg} gid={gid}"
        ) from exc
    return KvecStandardType1ProjectSurface(
        provenance=provenance,
        source_case=source_case,
        standard_case=standard_case,
        selected_operation_source_matrix=_complex_matrix_tuple(source_matrix),
        selected_operation_standard_matrix=_complex_matrix_tuple(standard_matrix),
        selected_operation_right_quotient=_complex_matrix_tuple(quotient),
    )
...
def dynamic_subduction_rows(
    ...
    *,
    ...
    for family in _selected_family_records(
        decoder, sg, basis_values, preferred_kparams
    ):
...
def dynamic_magnetic_subduction_occurrence_alias_rows(
    ...
    *,
    ...
    if requested_source_kparam is not None and parameter_key(requested_source_kparam) not in seen:
        requested_periodic = periodic_key(requested_source_kparam)
        matching_occurrence = next(
            (
                occurrence
        ...
        if matching_occurrence is not None:
            append_alias(requested_source_kparam, matching_occurrence)
    return tuple(aliases)
