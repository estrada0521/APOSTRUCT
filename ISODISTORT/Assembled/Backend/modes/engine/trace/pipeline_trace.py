...
from ISODISTORT.Assembled.Backend.modes.engine.project.mode_counts import little_records_for_k, mode_totals
from ISODISTORT.Assembled.Backend.modes.engine.project.entry import (
    project_entry_trace,
    project_local408_trace,
    site_get_irreps_trace,
)
from ISODISTORT.Assembled.Backend.modes.engine.trace.project_sources import (
...
def pipeline_trace(
    ...
    *,
    ...
    find_isotropy_orderparam = decoder.find_isotropy_orderparam_records_for_case(case) if case.k_params else ()
        ...
        "little_irreps": [
            {
                ...
                "lif": little.lif,
                "mode_total": totals.get(little.label, 0),
                "isotropy_rows": [
                    {
                        "row_id": item.row_id,
                ...
                if selected_basis_by_gid[little.gid] is None
                else {
                    "row_id": 0
                    if selected_isotropy_row_by_gid[little.gid] is None
                    else selected_isotropy_row_by_gid[little.gid].row_id,
                    "subgroup": 0
                    if selected_isotropy_row_by_gid[little.gid] is None
                    else selected_isotropy_row_by_gid[little.gid].subgroup,
                    "arms": 0
                    if selected_isotropy_row_by_gid[little.gid] is None
                    else selected_isotropy_row_by_gid[little.gid].arms,
                    "direction": 0
                    if selected_isotropy_row_by_gid[little.gid] is None
                    else selected_isotropy_row_by_gid[little.gid].direction,
                    "basis": list(selected_basis_by_gid[little.gid]),
                    "basis_pml_to_cinter": decoder.format_fraction_matrix(
                        decoder.transform_basis_rows(
                            selected_basis_by_gid[little.gid],
                            pml_to_cinter,
                        )
                    ),
                    "origin": []
                    if selected_isotropy_row_by_gid[little.gid] is None
                    else list(selected_isotropy_row_by_gid[little.gid].origin),
                },
