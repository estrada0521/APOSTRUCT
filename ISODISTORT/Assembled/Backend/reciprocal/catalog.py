...
def substitute_k_vector(vector: str, k_params: dict[str, str] | None) -> str:
    ...
    if "_single" not in k_params:
        public_values = [
            ...
            for key in ISODISTORT_K_PARAMS
...
def _isodistort_k_records(
    ...
    # convention, while the public ISODISTORT search page displays Kovalev
...
def _fmt_isodistort_k_component(
    records: tuple[tuple[Fraction, Fraction, Fraction], ...],
    axis: int,
    ...
    for index, vector in enumerate(records[1:], start=1):
        ...
        if value == 0:
            continue
        symbol = param_names.get(index, ISODISTORT_K_PARAMS[min(index - 1, 2)])
    ...
    if not param_names:
        for index, vector in enumerate(display_records[1:], start=1):
            for axis, value in enumerate(vector):
                if value:
                    param_names[index] = ISODISTORT_K_PARAMS[axis]
                    break
    return "(" + ",".join(_fmt_isodistort_k_component(display_records, axis, param_names) for axis in range(3)) + ")"
...
def _k_slot_info(dec: SourceTables, sg: int, kslot: int) -> dict[str, Any]:
    ...
    return {
        ...
        "isodistort_kvector": _fmt_isodistort_k_vector(basis[:dim + 1], lattice, symbol, label, dec, sg),
...
def _isodistort_star_vectors(dec: SourceTables, sg: int, kslot: int, dim: int) -> list[str]:
    sg_slot = (sg - 1) * 27 + kslot - 1
    count = int(dec.little["little_k_star_count"][sg_slot])
    if count <= 0:
        return []
    ...
    if dim == 0:
        ...
        for offset in range(count):
            ...
            arms.append(_fmt_isodistort_k_vector(record, lattice, symbol, label, dec, sg))
    ...
    for offset in range(count):
        ...
        if len(item) != 16:
            break
        arms.append(_fmt_isodistort_k_vector(_parse_k_record(item)[:dim + 1], lattice, symbol, label, dec, sg))
    return arms
