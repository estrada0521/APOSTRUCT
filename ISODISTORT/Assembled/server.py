"""Standalone localhost server for the ISODISTORT frontend and API."""
...
from ISODISTORT.Assembled import handle_api, handle_api_post
...
def _asset_path(path: str) -> Path | None:
    if path in {"/", "/index.html", "/tools/isodistort", "/tools/isodistort/"}:
        relative = "index.html"
    elif path.startswith("/tools/isodistort/"):
        relative = path.removeprefix("/tools/isodistort/")
    else:
...
def make_handler():
    class Handler(BaseHTTPRequestHandler):
        ...
        def do_GET(self) -> None:
            ...
            if parsed.path.startswith("/api/isodistort/"):
                endpoint = parsed.path.removeprefix("/api/isodistort/")
                try:
        ...
        def do_POST(self) -> None:
            ...
            if parsed.path == "/shutdown":
                ...
                return
            if not parsed.path.startswith("/api/isodistort/"):
                self._send(404, "Not Found")
                return
            try:
